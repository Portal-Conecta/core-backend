 ...
 * professores e representantes vinculados a turmas ativas do curso. Quando nenhum
 * filtro SHIFT é informado, todos os turnos de turmas ativas são considerados.</p>
 */
@Component
public class CourseScopeResolver {
     ...
     */
    public void insert(UUID notificationId, List<UUID> courseIds, EnumSet<TypeUser> types, EnumSet<Shift> shifts) {
        ...
        for (int i = 0; i < courseIds.size(); i += BATCH_SIZE) {
            ...
            userNotificationRepository.insertByCourseScope(notificationId, batch, typeNames, shiftNames);
        }
    }
}